...
{    
    ...
    // total_vefore_distribute = 0;
    // if (rank==0){
    //   for (int i = 1; i<n_proc; i++){
    //     MPI_Status m_status;
    //     MPI_Probe(i, 0, MPI_COMM_WORLD, &m_status);
    //     MPI_Get_count(&m_status, PARTICLE, &recieve_amount);
    //     total_vefore_distribute += recieve_amount;
    //     printf("recieve_amount: %d\n", recieve_amount);
    //     local.resize(recieve_amount);
    //     MPI_Recv(&local[0], recieve_amount, PARTICLE, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    //   }
    // } else {
    ...
    // for( int step = 0; step < 100; step++ )
    {
      // printf("step %d\n", step);
        ...
        // CHECKER, if first round and there are particles to more up or down
        // something wrong with initialization
        /*
        if (if_first){
          if ((move_up.size() != 0) || (move_down.size() != 0)){
            eixt(1);
          }
        }
        if_first = 0;
        */
        ...
        // CHECKER, if first processor has move_up or
        // last processor has move_down
        if ((rank == 0) && (move_up.size() != 0)) {
          // printf("ERROR: first processor has move up\n");
          exit(1);
        }
        if(((rank+1) == n_proc) && (move_down.size() != 0)){
          // printf("ERROR: last processor has move down\n");
          exit(1);
        }
        ...
        nlocal = local.size();
        // MPI_Reduce(&nlocal,&total,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
        // printf("total before send new local: %d\n", total);

        int move_up_totoal, move_down_total;
        int move_up_size = move_up.size();
        int move_down_size = move_down.size();
        // MPI_Reduce(&move_up_size,&move_up_totoal,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
        // printf("move up total: %d\n", move_up_totoal);
        // MPI_Reduce(&move_down_size,&move_down_total,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
        // printf("move down total: %d\n", move_down_total);

        // printf("start send new local\n");
        // first, even number rank will send the particles that moved to its
        ...
        nlocal = local.size();
        // MPI_Reduce(&nlocal,&total,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
        // printf("total after send new local: %d\n", total);
        ...
        nlocal = local.size();
        // MPI_Reduce(&nlocal,&total,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
        // printf("total before send new ghost: %d\n", total);

        int edge_up_totoal, edge_down_total;
        int edge_up_size = edge_up.size();
        int edge_down_size = edge_down.size();
        // MPI_Reduce(&edge_up_size,&edge_up_totoal,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
        // printf("edge up total: %d\n", edge_up_totoal);
        // MPI_Reduce(&edge_down_size,&edge_down_total,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
        // printf("edge down total: %d\n", edge_down_total);

        // printf("start sending ghost\n");
        ...
        // CHECKER, for the first processor, ghost from up should be 0
        // and for the last processor, ghost from down should be 0
        // for (int j = 0; j<ghost_from_down.size(); j++){
        //   printf("DOWN, ghost from processor %d at row %d\n", get_processor_number_for_particle(ghost_from_down[j], rowsofbin_per_proc, bins_per_row), get_row(ghost_from_down[j], bins_per_row));
        // }
        // for (int j = 0; j<ghost_from_up.size(); j++){
        //   printf("UP, ghost from processor %d at row %d\n", get_processor_number_for_particle(ghost_from_up[j], rowsofbin_per_proc, bins_per_row), get_row(ghost_from_up[j], bins_per_row));
        // }
