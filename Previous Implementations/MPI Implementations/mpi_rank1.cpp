...
int determin_edge(particle_t &p, int rank, int rowsofbin_per_proc, int n_proc, int bins_per_row){
  ...
  int lower_edge = (rank+1) * rowsofbin_per_proc;
...
{    
    ...
    // receive, if not master, dynamic receive amount by using MPI_Probe
      ...
      printf("before MPI_Probe\n");
      MPI_Probe(1, 0, MPI_COMM_WORLD, &m_status);
      MPI_Get_count(&m_status, MPI_INT, &recieve_amount);
      local.resize(recieve_amount);
      printf("recieve_amount at rank %d is: %d\n", rank, recieve_amount);
      MPI_Recv(&local[0], recieve_amount, PARTICLE, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    for( int step = 0; step < NSTEPS; step++ )
    {
      printf("step %d\n", step);
        ...
        */
        ...
        nlocal = local.size();
        MPI_Reduce(&nlocal,&total,1,MPI_INT,MPI_SUM,0,MPI_COMM_WORLD);
        printf("total after send new ghost: %d\n", total);

        // go over local, find edge up and down to be send as ghost
        for (int i; i<local.size(); i++) {
