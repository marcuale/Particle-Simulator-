...
void place_all_particles(vector<particle_t> *particle_bins, vector<particle_t> *partial_bins, int number_of_bins){
	for(int i = 0; i < number_of_bins; i++){
		omp_set_lock(&(bin_locks[i]));
        particle_bins[i].insert(particle_bins[i].end(),partial_bins[i].begin(),partial_bins[i].end());
		omp_unset_lock(&(bin_locks[i]));
	}
}

void place_partial_particles(vector<particle_t> *particle_bins, particle_t *particles, int start, int end){
	int particle_bin_number;
...
void place_all_particles_lock(vector<particle_t> *particle_bins, particle_t *particles, int start, int end){
	int particle_bin_number;
	for(int i = start; i < end; i++){
		particle_bin_number = get_bin_number(particles[i]);
		omp_set_lock(&(bin_locks[particle_bin_number]));
		particle_bins[particle_bin_number].push_back(particles[i]);
		omp_unset_lock(&(bin_locks[particle_bin_number]));
	}
}
//
...
{   
    ...
    //omp_init_lock(&bin_writelock);

	// array of locks test
	bin_locks = (omp_lock_t *) malloc(number_of_bins * sizeof(omp_lock_t));
    ...
    #pragma omp parallel private(dmin) shared(bin_locks)
    {
    int nthr = omp_get_num_threads();
    int tid = omp_get_thread_num();
    int chunk = n / nthr;
	//vector<particle_t> *cur_particle_bins = new vector<particle_t>[number_of_bins];
    ...
    for( int step = 0; step < 1000; step++ )
    {
        ...
        int start = tid * chunk;
        int end = (tid + 1) * chunk;
        // last thread, take all the rest
        if ((tid + 1) >= nthr) {
            end = n;
        }
		
		place_all_particles_lock(particle_bins, particles, start, end);
       // place_partial_particles(cur_particle_bins, particles, start, end);

       // omp_set_lock(&bin_writelock);
       // place_all_particles(particle_bins, cur_particle_bins, number_of_bins);
       // omp_unset_lock(&bin_writelock);
   ...
   // omp_destroy_lock(&bin_writelock);
